...
process bed2track {
    ...
    output:
    tuple val(group_name), val(description), path("*.bb"), emit: track

    script:

    description = "${group_name} (MUMERGE)"
...
process xlsx2track {
    ...
    output:
    tuple val(meta.group_name), val(description), path("*.bb"), emit: track

    script:
    window_size_str = meta.window_size == null ? "" : "_${meta.window_size}"
    description = "${meta.group_name} (${meta.method}${window_size_str})"
